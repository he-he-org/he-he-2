...
class VolunteerTemplate extends React.Component {
  renderTopics() {
    ...
    const currentTopicRoute =
      topic &&
      routes.volunteerPlaceTopic({
        language,
        place: place.frontmatter.name,
        topic: topic.frontmatter.name
      });

    return (
      <div className={styles.topics}>
        {topics.map(({ frontmatter }) => {
          let route = routes.volunteerPlaceTopic({
            language,
            place: place.frontmatter.name,
    ...
    if (articles.length === 0) {
      return (
        <div className={styles.noItemsMessage}>
          {t("pages_volunteer_no_articles_yet")}
        </div>
      );
    }
